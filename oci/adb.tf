...
resource "oci_database_autonomous_database" "autonomous_database" {
  ...
  display_name = format("adb-%s-%s-%s-%02d-%s", lower(local.current_region_key), lower(var.environment_code), lower(local.resource_prefix_shortname), count.index % var.numberOf_labs + 1, lower(format("%s%s%02d", local.db_workload_type, var.adb_version, floor(count.index / var.numberOf_labs + 1))))
