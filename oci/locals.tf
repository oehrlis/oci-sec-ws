...
locals {
  ...
  resource_prefix_shortname = var.label_prefix == "" ? lower(local.resource_shortname) : format("%s-%s", lower(var.label_prefix), lower(local.resource_shortname)) # Prefix resource shortname if label prefix is set
